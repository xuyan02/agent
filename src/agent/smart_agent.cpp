...
#include "agent/intuitive_agent.h"
#include "agent/routing_result_parse.h"
#include "agent/shallow_think_agent.h"
...
void SmartAgent::Run(std::string input,
  ...
  IntuitiveAgent intuitive(runtime(), ctx_);
  auto* self = this;
  ...
  intuitive.Run(
      *input_shared,
      [self, input_shared](std::string answer_json) mutable {
        ...
        ShallowThinkAgent shallow(self->runtime(), self->ctx_);
        shallow.Run(
